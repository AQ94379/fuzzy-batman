...
%f.delete();
%loc = "dtb/client/Dynamic/FixFire/themes/*.ftct";
for (%filename = findFirstFile(%loc); %filename !$= ""; %filename = findNextFile(%loc)) {
	...
	%d.close();
	%d.delete();
	%newname = getSubStr(%name, 0, 28);
	%shortname = strreplace(%filename, "dtb/client/Dynamic/FixFire/", "");
	%toolong = 0;
	if(strlen(%name) > 28){ %newname = %newname@"..."; %toolong = 1; }
	if(%toolong) {%newname = strreplace(%newname, " ...", "..."); if($themireport) error("THEMI: Name \""@%name@"\" too long for \""@%shortname@"\"!"); }
	themeList.addRow(%c++, %newname@"	"@%shortname);
	$themesettinglistnum[%c] = strreplace(strreplace(%filename, "dtb/client/Dynamic/FixFire/themes/", ""), ".ftct", "");
	if($themesettinglistnum[%c] == $Pref::Themi::ThemeSet) %settheme = %c;
}
...
//  $Pref::Themi::ThemeSet = $themesettinglistnum[%row + 1];
  saveThemes();
}
}
...
$Pref::Themi::ThemeSet = $themesettinglistnum[%i + 1];
// %i = themeList.getSelectedRow();
